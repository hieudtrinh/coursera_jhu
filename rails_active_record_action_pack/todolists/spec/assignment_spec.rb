      ...
      context "Generate four(4) Model Classes and Properties" do
        context "rq02.1 User Model" do
          ...
          it "User database structure in place" do
            ...
            expect(User.columns_hash["username"].type).to eq :string
            expect(User.columns_hash["password_digest"].type).to eq :string
            expect(User.columns_hash["created_at"].type).to eq :datetime
            expect(User.columns_hash["updated_at"].type).to eq :datetime
        ...
        context "rq02.2 Profile Model" do
          ...
          it "Profile database structure in place" do
            ...
            expect(Profile.columns_hash["gender"].type).to eq :string
            expect(Profile.columns_hash["birth_year"].type).to eq :integer
            expect(Profile.columns_hash["first_name"].type).to eq :string
            expect(Profile.columns_hash["last_name"].type).to eq :string
            expect(User.columns_hash["created_at"].type).to eq :datetime
            expect(User.columns_hash["updated_at"].type).to eq :datetime
        ...
        context "rq02.3 TodoList Model" do
          ...
          it "TodoList database structure in place" do
            ...
            expect(TodoList.columns_hash["list_name"].type).to eq :string
            expect(TodoList.columns_hash["list_due_date"].type).to eq :date
            expect(TodoList.columns_hash["created_at"].type).to eq :datetime
            expect(TodoList.columns_hash["updated_at"].type).to eq :datetime
        ...
        context "rq02.4 TodoItem Model" do
          ...
          it "TodoItem database structure in place" do
            ...
            expect(TodoItem.columns_hash["due_date"].type).to eq :date
            expect(TodoItem.columns_hash["title"].type).to eq :string
            expect(TodoItem.columns_hash["description"].type).to eq :text            
            expect(TodoItem.columns_hash["completed"].type).to eq :boolean
            expect(TodoItem.columns_hash["created_at"].type).to eq :datetime
            expect(TodoItem.columns_hash["updated_at"].type).to eq :datetime
